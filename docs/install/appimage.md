...
The CPU AppImage contains the Lexaloud native binaries (`lexaloud` Rust daemon+CLI
and `lexaloud-ui` Qt 6 UI), the CPU ONNX Runtime, eSpeak data, PortAudio,
and the clipboard helper programs used by the selection and clipboard
commands. It does not require a toolchain on the host.

Download the `Lexaloud-...-x86_64.AppImage` asset from the releases page,
then run:
...
- runs the speech daemon as part of the app — no systemd unit, no
  terminal commands, and no host toolchain.
...
The Qt 6 runtime is bundled (allowlisted plugins + libs), so the tray icon works out of
...
The AppImage is currently CPU-only. A CUDA AppImage may be provided later;
the source build with `--backend cuda12` remains available for NVIDIA hosts.
